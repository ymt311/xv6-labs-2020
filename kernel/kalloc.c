...
struct {
  ...
  struct spinlock stealing_lock;  // 给每个核心设置一个stealing锁
  struct run *freelist;
...
kfree(void *pa)
{
  ...
  int cpu = cpuid();   // 函数cpuid返回当前的核心编号
...
kalloc(void)
{
  ...
  if(!kmem[cpu].freelist){  // 如果当前cpu无空闲物理页，则需要从其他cpu的空闲页表进行窃取

    int steal_left = 64; // steal 64 pages from other cpu(s)
    for(int i=0;i<NCPU;i++){
      if(i == cpu) continue;

      acquire(&kmem[cpu].stealing_lock);  // steal之前先拿steal锁再放弃自己的freelist锁
      release(&kmem[cpu].lock);

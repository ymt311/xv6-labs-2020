...
struct {
  struct spinlock eviction_lock;   // 将驱逐+重分配的过程限制为单线程
  struct buf buf[NBUF];

  // 建立一个从 blockno 到 buf 的哈希表，并为每个桶(每个散列桶存储具有相同哈希值的元素)单独加锁
...
bget(uint dev, uint blockno)
{
  ...
  for(int i = 0; i < NBUFMAP_BUCKET; ++i){
    ...
    if(!newfound){  // 当前这个桶里没有最近最久未使用过的buf
      release(&bcache.bufmap_locks[i]);
    }else{
      // holding_bucket != -1，说明之前已经找到一个buf了，但出现了更适合替换的buf，释放之前的holding_bucket
